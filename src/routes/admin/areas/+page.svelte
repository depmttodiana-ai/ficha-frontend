...
<div class="space-y-4 md:space-y-6">
	<div class="flex flex-col sm:flex-row sm:items-center justify-between gap-2 sm:gap-4">
		<h2 class="text-lg md:text-xl font-semibold text-slate-800">Áreas</h2>
			...
			<table class="min-w-[400px] w-full divide-y divide-slate-200">
				<thead class="bg-slate-50">
					<tr>
						...
						<th class="px-2 md:px-4 py-2 md:py-3 text-left text-xs font-semibold text-slate-500 uppercase">Código</th>
				...
				<tbody class="divide-y divide-slate-200">
					...
					{:else if areas.length === 0}
						<tr><td colspan="3" class="px-2 md:px-4 py-8 md:py-12 text-center text-slate-400 text-sm">Sin áreas registradas</td></tr>
					{:else}
						{#each areas as a}
							<tr class="hover:bg-slate-50">
								<td class="px-2 md:px-4 py-2 md:py-3 text-xs md:text-sm font-medium text-slate-800">{a.nombre}</td>
								<td class="px-2 md:px-4 py-2 md:py-3 text-xs md:text-sm text-slate-600">{a.codigo || '—'}</td>
								<td class="px-2 md:px-4 py-2 md:py-3 text-xs md:text-sm whitespace-nowrap">
									<button onclick={() => openEdit(a.id)} class="text-blue-600 hover:underline mr-2 md:mr-3">Editar</button>
									<button onclick={() => handleDelete(a.id)} class="text-red-600 hover:underline">Eliminar</button>
								</td>
							</tr>
						{/each}
					{/if}
...
<Modal bind:open={showModal} title={editing ? 'Editar Área' : 'Nueva Área'} onConfirm={handleSave}>
	<div class="space-y-4">
		...
		<div>
			...
			<input type="text" bind:value={formCodigo} class="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm" />
		</div>
		<div>
