...
<div class="space-y-4 md:space-y-6">
	<div class="flex flex-col sm:flex-row sm:items-center justify-between gap-2 sm:gap-4">
		<h2 class="text-lg md:text-xl font-semibold text-slate-800 dark:text-slate-100">Clasificaciones</h2>
			...
			<table class="min-w-[400px] w-full divide-y divide-slate-200 dark:divide-slate-700">
				<thead class="bg-slate-50 dark:bg-slate-700">
					<tr>
						...
						<th class="px-2 md:px-4 py-2 md:py-3 text-left text-xs font-semibold text-slate-500 dark:text-slate-300 uppercase">Descripción</th>
				...
				<tbody class="divide-y divide-slate-200 dark:divide-slate-700">
					...
					{:else if clasificaciones.length === 0}
						<tr><td colspan="3" class="px-2 md:px-4 py-8 md:py-12 text-center text-slate-400 dark:text-slate-500 text-sm">Sin clasificaciones</td></tr>
					{:else}
						{#each clasificaciones as c}
							<tr class="hover:bg-slate-50 dark:hover:bg-slate-700">
								<td class="px-2 md:px-4 py-2 md:py-3 text-xs md:text-sm font-medium text-slate-800 dark:text-slate-100">{c.nombre}</td>
								<td class="px-2 md:px-4 py-2 md:py-3 text-xs md:text-sm text-slate-600 dark:text-slate-300">{c.descripcion || '—'}</td>
								<td class="px-2 md:px-4 py-2 md:py-3 text-xs md:text-sm whitespace-nowrap">
									<button onclick={() => openEdit(c.id)} class="text-blue-600 dark:text-blue-400 hover:underline mr-2 md:mr-3">Editar</button>
									<button onclick={() => handleDelete(c.id)} class="text-red-600 dark:text-red-400 hover:underline">Eliminar</button>
								</td>
							</tr>
						{/each}
					{/if}
